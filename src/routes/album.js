...
router.post('/manage/photos/:id/delete', requireFamily, async (req, res) => {
  ...
  await db.run('DELETE FROM album_comments WHERE photo_id = ?', id);
  await db.run('DELETE FROM emoji_reactions WHERE photo_id = ?', id);
  await db.run('DELETE FROM album_photos WHERE id = ?', id);

  req.session.flash = { type: 'success', text: 'Photo deleted.' };
  return res.redirect('/album/manage');
});

router.get('/', requireAlbumAccess, async (req, res) => {
  ...
  const commentReactionRows = await db.all(
...
router.post('/comment', requireAlbumAccess, async (req, res) => {
  ...
  const photo = await db.get('SELECT title FROM album_photos WHERE id = ?', photoId);
...
router.post('/comment-reaction', requireAlbumAccess, async (req, res) => {
  const actor = getAlbumActor(req);
  ...
  const comment = await db.get(
    'SELECT id FROM album_comments WHERE id = ? AND deleted_at IS NULL',
    commentId
  );
  if (!comment) {
...
router.post('/emoji', requireAlbumAccess, async (req, res) => {
  const actor = getAlbumActor(req);
  const photoId = Number(req.body.photoId);
