...
router.get('/', (req, res) => {
  ...
  const members = orderedNames.map((name) => {
    ...
    return {
      ...
      profileImage: row ? row.profile_image || '' : ''
    };
  });

  const heroPhotos = db
    .prepare('SELECT id, title, image_url FROM album_photos ORDER BY highlight_order ASC, id DESC LIMIT 12')
    .all();
  ...
  const landingBackgroundImageRow = db
    .prepare("SELECT setting_value FROM api_settings WHERE setting_key = 'landing_background_image'")
    .get();
  const landingBackgroundImage = landingBackgroundImageRow
